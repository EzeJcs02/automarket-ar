    ...
    <div className="page-wrapper">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px' }}>
        ...
        <div style={{ padding: '3rem 4rem', borderRight: '1px solid var(--gray2)' }}>
          ...
          {/* FOTO PRINCIPAL */}
          <div style={{ width: '100%', height: '450px', background: 'linear-gradient(135deg, var(--gray2), var(--gray1))', borderRadius: 'var(--radius-lg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '80px', marginBottom: '1rem', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
            {fotos.length > 0
          ...
          {fotos.length > 1 && (
            <div style={{ display: 'flex', gap: '10px' }}>
              {fotos.map((f, i) => (
                <div key={i} onClick={() => setFotoIdx(i)} style={{ width: '90px', height: '65px', borderRadius: 'var(--radius)', overflow: 'hidden', border: `2px solid ${i === fotoIdx ? 'var(--accent)' : 'transparent'}`, cursor: 'pointer', opacity: i === fotoIdx ? 1 : 0.6, transition: 'all .2s' }}
        ...
        <div style={{ padding: '3rem 2rem', position: 'sticky', top: '58px', height: 'calc(100vh - 58px)', overflowY: 'auto' }}>
          ...
          {/* BLOQUE DE PRECIO PROLIJO Y BALANCEADO */}
          {(Number(auto.precio_ars) > 0 || Number(auto.precio_usd) > 0) && (
            <div style={{ padding: '1.25rem', background: 'var(--gray1)', borderRadius: 'var(--radius-lg)', marginTop: '2rem', marginBottom: '2rem', border: '1px solid var(--gray2)' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--gray4)', marginBottom: '8px', letterSpacing: '.1em', textTransform: 'uppercase' }}>
                PRECIO DE LISTA
              </div>
              ...
              {Number(auto.precio_usd) > 0 && (
                <div style={{ 
                  fontFamily: Number(auto.precio_ars) > 0 ? 'var(--font-body)' : 'var(--font-display)',
                  fontSize: Number(auto.precio_ars) > 0 ? '16px' : '40px', 
                  color: Number(auto.precio_ars) > 0 ? 'var(--accent)' : 'var(--white)', 
                  fontWeight: '600', 
                  marginTop: Number(auto.precio_ars) > 0 ? '8px' : '0', 
                }}>
          ...
          {/* CONTACTAR CONCESIONARIA (Cuadrito restaurado) */}
          {c && (
